...
func parseUsageStats(text string) []UsageStat {
	...
	for i := startIndex; i < len(lines); i++ {
		...
		if len(parts) >= 5 {
			usageStr := strings.TrimSuffix(parts[3], "%")
			if usagePercent, err := strconv.ParseFloat(usageStr, 64); err == nil && usagePercent > 0 {
				data = append(data, UsageStat{
					Pokemon:      parts[2],
...
func populateUsage() {
	...
	slog.Info("Creating usage_stats table if it doesn't exist...")
	_, err = pool.Exec(ctx, `
	...
	slog.Info("Fetching existing usage records to skip...")
	rows, err := pool.Query(ctx, "SELECT DISTINCT month, format, rating FROM usage_stats")
	if err != nil {
				...
				go func(file string) {
					...
					if count > 0 {
						...
						for k := 0; k < count; k++ {
							...
							usagesArr[k] = data[k].UsagePercent
						}

						_, err := pool.Exec(ctx, insertQuery, monthsArr, formatsArr, ratingsArr, pokemonsArr, usagesArr)
						if err != nil {
					...
					slog.Info(fmt.Sprintf("Inserted usage stats for %d pokemon in %s.", count, key))
					mu.Lock()
	...
	slog.Info("Finished populating usage stats.")
}
