...
function parseStats(text) {
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    if (parts.length >= 5) {
      const leadPercent = parts[3];
      if (parseFloat(leadPercent) > 0) {
        data.push({
          pokemon: parts[2],
          leadPercent: parseFloat(leadPercent)
        });
      }
    }
  }
...
async function run() {
  ...
  console.log("Creating leads_stats table if it doesn't exist...");
  await client.query(`
  ...
  console.log("Fetching existing leads records to skip...");
  const res = await client.query('SELECT DISTINCT month, format, rating FROM leads_stats');
  ...
  for (const month of months) {
    ...
    let monthHtml;
    try {
      monthHtml = await fetchHtml(`${BASE_URL}${month}/leads/`);
    } catch (e) {
      console.log(`Failed to fetch leads directory for ${month}, skipping.`);
      continue;
    }
    ...
    for (let i = 0; i < targetFiles.length; i += CONCURRENCY) {
      ...
      await Promise.all(batch.map(async (file) => {
        ...
        const targetUrl = `${BASE_URL}${month}/leads/${file}`;
        ...
        try {
          ...
          if (count > 0) {
            ...
            const leadPercentsArr = data.map(item => item.leadPercent);
            
            await client.query(insertQuery, [monthsArr, formatsArr, ratingsArr, pokemonsArr, leadPercentsArr]);
            
            console.log(`Inserted leads stats for ${count} pokemon in ${key}.`);
